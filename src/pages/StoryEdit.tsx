...
export function StoryEdit() {
  ...
  const fetchStory = async (storyId: string) => {
    ...
    try {
      ...
      setClassLevel(storyData.class_level || '')
      setCurrentFileUrl(storyData.file_url)
      setCurrentFileType(storyData.file_type)
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!user || !id) {
      setError('Authentication error or story ID missing.')
      showNotification('Authentication error or story ID missing.', 'error')
      return
    }

    if (!title.trim()) {
      setError('Please fill in the story title.')
      showNotification('Please fill in the story title.', 'error')
      return
    }

    if (!selectedFile && !currentFileUrl) {
      setError('Please select a file or ensure a current file exists.')
      showNotification('Please select a file or ensure a current file exists.', 'error')
      return
    }
    ...
    try {
      ...
      if (selectedFile) {
        ...
        newFileType = selectedFile.type
      }

      // Update story metadata in the 'stories' table
      ...
      setError(err.message)
      showNotification('Error updating story: ' + err.message, 'error')
    } finally {
      setSaving(false)
    }
  }
  ...
  if (error && error !== 'You do not have permission to edit this story.') {
    return (
      <ChildFriendlyBackground>
        <div className="p-6 pb-20 lg:pb-6">
          <div className="max-w-4xl mx-auto">
      ...
      <div className="p-6 pb-20 lg:pb-6">
        <div className="max-w-4xl mx-auto">
          ...
          <div className="bg-white/90 backdrop-blur-sm rounded-xl shadow-lg p-6 border border-white/50">
            <div className="mb-6">
              <h1 className="text-3xl font-bold text-gray-900 mb-2 drop-shadow-sm">Edit Story 📖</h1>
              <p className="text-gray-700 drop-shadow-sm">
                Update the details or replace the file for this story.
              </p>
            </div>
              ...
              <div>
                ...
                {currentFileUrl && !selectedFile && (
                  <p className="mb-2 text-sm text-gray-600">
                    Current file: <a href={currentFileUrl} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline">View Current File</a> ({currentFileType})
                  </p>
                )}
                <input
                  type="file"
